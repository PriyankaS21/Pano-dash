...
children_container1 = [
    html.Div(className='row', children=[
        ...
        ], style={'width': '60%', 'padding': '10px', 'padding-left': '290px'}),
    ]),
...
children_container2 = [
    ...
    ], style={'width': '35%', 'padding': '10px', 'margin-left': '580px'}),
...
def resultreturn(dateval, fstrel, secenv, thrddayp, frthdiff, lastradio):
    ...
    if datesdict[dateval] == 'Today':
        datevalnow = datetime.strftime(
            datetime.date(datetime.now()), '%m-%d-%Y')
    else:
        datevalnow = datetime.strftime(datetime.strptime(
            sevenlist[dateval], '%m-%d-%Y'), '%m-%d-%Y')
